function pyra = featpyramid(im, model, algorithm, verbose, padx, pady)
...
%   imshow(mat2gray(scaled));
  if extra_interval > 0
    % Optional (sbin/4) x (sbin/4) features
    pyra.feat{i} = newFeatures(scaled, sbin/4, algorithm, verbose);
    pyra.scales(i) = 4/sc^(i-1);
  end
  % (sbin/2) x (sbin/2) features
  pyra.feat{i+extra_interval} = newFeatures(scaled, sbin/2, algorithm, verbose);
  pyra.scales(i+extra_interval) = 2/sc^(i-1);
  % sbin x sbin HOG features
  pyra.feat{i+extra_interval+interval} = newFeatures(scaled, sbin, algorithm, verbose);
  ...
  for j = i+interval:interval:max_scale
    ...
    pyra.feat{j+extra_interval+interval} = newFeatures(scaled, sbin, algorithm, verbose);
