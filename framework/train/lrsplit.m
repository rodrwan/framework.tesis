function [A, B] = lrsplit(model, pos, algorithm, verbose)
...
dim = numel(newFeatures(double(warped{1}), model.sbin, algorithm, verbose));
F = zeros(numpos, dim);
for i = 1:numpos
  tic_toc_print('%s %s: lrsplit features: %d/%d\n', ...
                procid(), model.class, i, length(warped));
  F(i,:) = reshape(newFeatures(double(warped{i}), model.sbin, algorithm, verbose), [1 dim]);
end
