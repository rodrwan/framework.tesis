...
try
  ...
  for i = 1:n
    %
    % Es aqui donde se crea la primera instancia del modelo, apartir de
    % este punto todos los datos obtenidos por el entrenamiento son
    ...
    inds = lrsplit(models{i}, spos{i}, algorithm, verbose);
...
try
  ...
  for i = 1:n
    ...
    % models{i} = train(models{i}, spos{i}, neg_small, false, false, 4, 3, ...
                      % max_num_examples, fg_overlap, 0, false, ...
                      % ['lrsplit2_' num2str(i)]);
    models{i} = train(models{i}, spos{i}, neg_small, false, false, 1, 1, ...
                      max_num_examples, fg_overlap, 0, false, ...
                      ['lrsplit2_' num2str(i)]);
  end
...
try
  ...
  % 1 5
  model = train(model, impos, neg_small, false, false, 1, 1, ...
...
try
  ...
  % 1 5
  model = train(model, impos, neg_large, false, false, 1, 1, ...
