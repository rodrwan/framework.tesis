function model = train(model, pos, neg, warp, randneg, iter, negiter, max_num_examples, fg_overlap, num_fp, cont, tag, C)
...
for t = 1:iter
  ...
  %
  % Solo se ejecuta luego de la primera iteracion, o si es necesario
  % mediante los parametros de la funcion
  %
  if ~cont || t > 1
    ...
    if warp
      [num_entries_added, num_examples_added] = poswarp(t, model, pos, algorithm, verbose);
      fusage = num_examples_added;
      component_usage = num_examples_added;
    else
      th = tic();
      disp('poslatent process running..');
      %
      % Esta funcion computa los ejemplos positivos
      % t: iteracion actual
      % iter: training iteration
      % model: model struct to save data
      % pos: separacion de los ejemplos que estan en primer plano -> ver
      % pascal_train.m
      % fg_overlap: 0.7 viene dado por el archivo de configuracion
      % voc_config.m
      % num_fp: no se especifica que es pero su valor inicial es cero
      %
      [num_entries_added, num_examples_added, fusage, component_usage, scores] = poslatent(t, iter, model, pos, fg_overlap, num_fp, algorithm, verbose);
      th = toc(th);
      ...
      for tt = 1:t
        fprintf('>> poslatent: positive loss before: %f, after: %f, ratio: %f\n', pos_loss(tt,1), pos_loss(tt,2), pos_loss(tt,2)/pos_loss(tt,1));
      end
    ...
    for i = 1:length(model.rules{model.start})
      fprintf('  component %d got %d/%d (%.2f%%) examples\n', i, component_usage(i), num_examples_added, 100*component_usage(i)/num_examples_added);
    end
  ...
  %
  % desde aqui se procesan los datos que hay fuera de las ROI, es decir
  % todo el background.
  % Esto se utiliza en la primera fase de entrenamiento.
  ...
  for tneg = 1:negiter
    ...
    if datamine
      ...
      if randneg
        %
        % se generan datos aleatorios extraidos del background de la imagen
        %
        [num_entries_added, num_examples_added] = negrandom(t, model, neg, max_num_examples-num_examples, algorithm, verbose);
        num_entries = num_entries + num_entries_added;
        num_examples = num_examples + num_examples_added;
        %
        % filtro usado
        % en este caso son los ejemplos generados de manera random por la
        % funcion negrandom.
        %
        fusage = num_examples_added;
      else
        th = tic();
        %
        % esta funcion debería generar filtros y entregar el score de estos
        % por cada uno de los background extraidos desde el punto origen.
        % Esta funcion busca los posibles ejemplos negativos.
        %
        [num_entries_added, num_examples_added, negpos, fusage, scores, complete] = neghard(tneg, negiter, model, neg, bytelimit, negpos, max_num_examples-num_examples, algorithm, verbose);
        ...
        %
        num_examples = num_examples + num_examples_added;
        % se agrega el score obtenido por los ejemplos negativos a la
        % variable hinge
        hinge = max(0, 1+scores);
        %
    ...
    %{{{ Optimize the convex slave problem
      ...
      % !!ACA ESTA LA MAGIA!!
      % !!!!!!!!!!!!!!!!!!!!!
      % funcion objetivo que se encuentra en el archivo obj_func.cc
      % el codigo esta algo desordenado y es dificil de entender
      %
      obj_func = @(x) fv_obj_func(x, 2*pool_size);
      % Vectorize the model parameters and get the box constraints
      w = cat(1, blocks{:});
      lb = cat(1, lb{:}); % lower bound
      ub = inf*ones(size(lb)); % uper bound
      ...
      th = tic;
      % se utilizala funcion objetivo, es importante!
      ...
      model_name = [model.class '_model_' tag '_' num2str(t) '_' num2str(tneg)];
    ...
    %{{{ Maintain the feature vector cache
      ...
      fprintf('cached %d (%d) positive and %d (%d) negative examples (entries)\n', cached_pos_examples, cached_pos_entries, cached_neg_examples, cached_neg_entries);

      % Count number of support vectors
      I = find((info.is_belief == 0)&(info.is_mined == 0)& (info.is_unique == 1)&(info.margins < 0.000001));
      num_sv = size(unique([info.dataid(I) info.scale(I) info.x(I) info.y(I)], 'rows'), 1);
      fprintf('%d foreground support vectors\n', num_sv);
      I = find((info.is_belief == 0)&(info.is_mined == 1)& (info.is_unique == 1)&(info.margins < 0.000001));
      num_sv = size(unique([info.dataid(I) info.scale(I) info.x(I) info.y(I)], 'rows'), 1);
...
function [num_entries, num_examples] = poswarp(t, model, pos, algorithm, verbose)
...
for i = 1:numpos
  ...
  im = warped{i};
%   imshow(im);
  feat = newFeatures(double(im), model.sbin, algorithm, verbose);
  key = [i 0 0 0];
  bls = [obl; fbl] - 1;
  feat = [model.features.bias; feat(:)];
  fv_cache('add', int32(key), int32(bls), single(feat), int32(is_belief), int32(is_mined), loss);
...
% we create virtual examples by flipping each image left to right
function [num_entries, num_examples, fusage, component_usage, scores] = poslatent(t, iter, model, pos, fg_overlap, num_fp, algorithm, verbose)
% carga configuracion base desde voc_config.m
conf = voc_config();
% intervalo para los ejemplos en primer plano = 5
model.interval = conf.training.interval_fg;
% numero de ejemplos
numpos = length(pos);
% se carga la cantidad de pixeles
pixels = model.minsize * model.sbin / 2;
minsize = prod(pixels);
% variable que contiene el filtro, inicializada en cero
...
for i = 1:batchsize:numpos
  ...
  for k = 1:thisbatchsize
    ...
    % PYRAMID LOGIC
    % PIRAMIDEEE!!!!!!
    % recibe como parametro una subseccion del bounding box de la imegen
    % descrito en las anotaciones del set de entrenamiento.
    im = color(imreadx(pos(j)));

    % se extraen las bounding box definidas para los ejemplos positivos
    [im, boxes] = croppos(im, pos(j).boxes);
%     imshow(im);
    % se obtiene la piramide y el modelo de esa parte
    % model_dp es el modelo obtenido luego de la programacion dinamica.
    % pyra es seteado en featpyramid.m
    [pyra, model_dp] = gdetect_pos_prepare(im, model, boxes, fg_overlap, algorithm, verbose);
    data(k).pyra = pyra;

    % process each box in the imagex
    ...
    for b = 1:num_boxes
      ...
      % bs: filter bounding boxes || bounding box sobre el objeto
      % en los bounding boxes
      % trees: derivation trees
      %
      [ds, bs, trees] = gdetect_pos(data(k).pyra, model_dp, 1+num_fp, fg_box, fg_overlap, bg_boxes, 0.5);
      % pause(0.1);
%       imshow(im);showboxes(im, bs); % mostrar roi con filtro
  ...
  for k = 1:thisbatchsize
    ...
    for b = 1:length(pos(j).dataids)
      ...
      %
      % reemplaza la variblae bs con el contenido que tenia mas la cuenta
      % de los filtros obtenidos
      %
      bs = gdetect_write(data(k).pyra, model, data(k).boxdata{b}.bs, data(k).boxdata{b}.trees, true, dataid);
      if ~isempty(bs)
        % array de filtros obtenidos en la deteccion.
...
% son todos aquellos los que tienen un valor de score negativo menor que el
% (5)(6)(7)(8) ?
function [num_entries, num_examples, j, fusage, scores, complete] = neghard(t, negiter, model, neg, maxsize, negpos, max_num_examples, algorithm, verbose)
conf = voc_config();
...
%     pause(0.1);
%     imshow(im);showboxes(im, bs); % mostrar roi con filtro
    data{k}.bs = bs; % array with bounding boxes filters (maybe)
    data{k}.pyra = pyra;
    data{k}.trees = trees;
  end
  % write feature vectors sequentially
  ...
  for k = 1:thisbatchsize
    j = inds(i+k-1);
    dataid = neg(j).dataid;
    bs = gdetect_write(data{k}.pyra, model, data{k}.bs, data{k}.trees, false, dataid, maxsize, max_num_examples-num_examples);
    if ~isempty(bs)
...
function [num_entries, num_examples] = negrandom(t, model, neg, maxnum, algorithm, verbose)
numneg = length(neg);
...
for i = 1:numneg
  ...
  try
    ...
    neg(i).im
    error('Cannot execute feature script');
  end
  if size(feat,2) > rsize(2) && size(feat,1) > rsize(1)
    for j = 1:rndneg
      ...
      fv_cache('add', int32(key), int32(bls), single(f), int32(is_belief), int32(is_mined), loss);
