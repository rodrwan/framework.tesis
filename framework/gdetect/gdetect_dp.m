...
% take pointwise max over scores for each rule with s as the lhs (left hand
% side)
rules = model.rules{s};
score = rules(1).score;
...
for level = 1:length(pyra.feat)
  ...
  if size(pyra.feat{level},3) == 32
    ...
    % 0: features en el nivel level.
    % 1: filtros reshepiados.
    % 2: valor de inicio
    % 3: cantidad de filtros (valor de fin).
    % esto se calcula solo con un nivel valido.
